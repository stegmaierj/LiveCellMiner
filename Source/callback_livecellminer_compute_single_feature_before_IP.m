...
if (isempty(next_function_parameter))
    prompt = {'Enter frames before IP:'};
    dlgtitle = 'Specify number of frames before IP to extract the single feature...';
    dims = [1 35];
    definput = {'10'};
...
for f=selectedFeatures
    ...
    if (strcmp(kill_lz(dorgbez(end,:)), 'y'))
        dorgbez = char(dorgbez(1:end-1, :), sprintf('%s_%02dFramesBeforeIP', kill_lz(var_bez(f,:)), numFrames));
    else
        dorgbez = char(dorgbez, sprintf('%s-%02dFramesBeforeIP', kill_lz(var_bez(f,:)), numFrames));
    end
    ...
    for i=1:size(d_orgs,1)
        ...
        currentIPTransition = find(d_orgs(i, :, syncFeatureIndex) == 1, 1, 'last');

        if (isempty(currentIPTransition) || (currentIPTransition-numFrames) < 1)
            d_orgs(i, :, syncFeatureIndex) = -1; %#ok<SAGROW> 
            continue;
        end

        %% find normalization time point
        featureBeforeIP = squeeze(d_orgs(i,currentIPTransition-numFrames,f));

        %% set the Sister distance feature
        d_org(i, newFeatureIndex) = featureBeforeIP;
    end
end