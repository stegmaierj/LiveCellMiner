...
if (useAutoRejection == true)
    ...
    if (~exist('maskedImageCNNFeatures', 'var'))
        callback_chromodec_load_image_files;
    end    
...
synchronizationIndex = callback_chromodec_find_time_series(var_bez, 'manualSynchronization');
...
synchronizationIndex = callback_chromodec_find_time_series(var_bez, 'manualSynchronization'); 
areaIndex = callback_chromodec_find_time_series(var_bez, 'Area');
circularityIndex = callback_chromodec_find_time_series(var_bez, 'Circularity');
meanIntensityIndex = callback_chromodec_find_time_series(var_bez, 'MeanIntensity');
stdIntensityIndex = callback_chromodec_find_time_series(var_bez, 'StdIntensity');
entropyIndex = callback_chromodec_find_time_series(var_bez, 'Entropy');
...
manuallyConfirmedIndex = callback_chromodec_find_single_feature(dorgbez, 'manuallyConfirmed');
...
for i=ind_auswahl'%1:2:size(d_orgs,1)
    ...
    if (exist('classificationLSTM', 'var') && useAutoRejection == true)
        ...
        if (useClassicalSync == false)
            ...
            autoSyncPredictionHMMCorrected = callback_chromodec_perform_HMM_prediction(autoSyncPrediction);
