...
function [featureNames, resultMatrix, deletionIndices, rawImagePatches, maskImagePatches, rawImagePatches2] = callback_chromodec_cell_patch_extraction(d_orgs, candidateList, parameters)
            ...
            if cellIDs(j,1) <= min(maxCellID, numLabels)
                ...
                if i >= (startTimePoints(j)+1) && i <= (endTimePoints(j)+1)
                    ...
                    if ~ismember(currentIndex, deletionIndices)
                        ...
                        if (isempty(parameters.maskFolder))
                            croppedMask = uint16(callback_chromodec_segment_center_nucleus(croppedImage));
                        else
                        ...
                        [featureNames, features] = callback_chromodec_extract_nucleus_features(croppedImage, croppedMask);
