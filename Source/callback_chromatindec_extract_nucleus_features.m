...
function [featureNames, featureVector] = callback_chromatindec_extract_nucleus_features(rawImage, maskImage)
    ...
    %% combine the shape features, predicted probabilities and the Haralick texture features
    featureNames = char(featureNames1, featureNames2, featureNames3);
    featureVector = [featureVector1, transpose(featureVector2), probabilityInter(1), probabilityMeta(1)];
