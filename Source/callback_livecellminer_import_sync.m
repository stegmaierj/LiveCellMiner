...
for eOld=unique(code_alle_old(:,experimentIndexOld))'
    for pOld=unique(code_alle_old(:,positionIndexOld))'
        ...
        successFlag = false;
        oldIndices = find(code_alle_old(:,experimentIndexOld) == eOld & code_alle_old(:,positionIndexOld) == pOld);
        ...
        for eNew=unique(code_alle(:,experimentIndexNew))'
            for pNew=unique(code_alle(:,positionIndexNew))'
                ...
                if (successFlag == true || strcmp(oldExperiment, newExperiment) == 0 || strcmp(oldPosition, newPosition) == 0)
                    continue;
                else
                    newIndices = find(code_alle(:,experimentIndexNew) == eNew & code_alle(:,positionIndexNew) == pNew);
                    ...
                    if (length(newIndices) == length(oldIndices))
                        ...
                        successFlag = true;
                        disp(['Successfully copied synchronization for '  newExperiment ' / ' newPosition]);
                    else
                        disp(['Matching entries found for ' newExperiment ' / ' newPosition ' but number of cell deviates (' num2str(length(oldIndices)) ' vs. ' num2str(length(newIndices)) ')!!']); 
                    end
                end
            end
        end
        ...
        if (successFlag == false)
            disp(['No matching entries found for ' oldExperiment ' / ' oldPosition '!!']); 
        end
    end
end
