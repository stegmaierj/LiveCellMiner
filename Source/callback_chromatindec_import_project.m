...
function [] = callback_chromatindec_import_project(parameters)
    ...
    XPIWITCommand = ['./XPIWIT.sh --output "' outputFolderSeeds '" --input "0, ' inputFolder ', 2, float" --xml "' parameters.XPIWITDetectionPipeline '" --seed 0 --lockfile off --subfolder "filterid, filtername" --outputformat "imagename, filtername" --end'];
    ...
    if (parameters.useCellpose == true)
        ...
        CELLPOSECommand = ['python -m cellpose --dir ' parameters.inputFolderCellpose ' --chan 0 --model_dir ' parameters.CELLPOSEModelDir CELLPOSEFilter ' --pretrained_model nuclei --output_dir ' outputFolderCellpose ' --diameter 30 --use_gpu --save_png'];
        system(CELLPOSECommand);
    end

    %% perform feature extraction, tracking and project generation
    callback_chromatindec_perform_detection_and_tracking(parameters);
end