...
function [] = callback_chromodec_perform_detection_and_tracking(parameters)
    ...
    [d_orgs, ~] = callback_chromodec_perform_seed_detection(parameters);

    %% refine detected seeds based on the weighted centroid on the previous frame
    [d_orgs] = callback_chromodec_refine_detected_seeds(d_orgs, parameters);

    %[d_orgs_new] = PerformTracking(d_orgs, settings);
    [d_orgs] = callback_chromodec_perform_backwards_tracking(d_orgs, parameters);
    ...
    [motherList, daughterList, motherDaughterList] = callback_chromodec_candidate_list_generation(d_orgs, parameters);
    candidateList = [motherList; daughterList];

    %% Parameters for cell patch extraction
    [featureNames, featureMatrix, deletionIndices, rawImagePatches, maskImagePatches, rawImagePatches2] = callback_chromodec_cell_patch_extraction(d_orgs, candidateList, parameters);

    %% combine the mother and daughter features to the final feature matrix
    [finalFeatureMatrix, originalIds, finalRawImagePatches, finalMaskImagePatches, finalRawImagePatches2] = callback_chromodec_combine_mother_daughter_tracks(featureMatrix, motherList, daughterList, motherDaughterList, deletionIndices, rawImagePatches, maskImagePatches, rawImagePatches2, parameters);
    ...
    xposID = callback_chromodec_find_time_series(featureNames, 'xpos');
    yposID = callback_chromodec_find_time_series(featureNames, 'ypos');
    minorAxisLenghtID = callback_chromodec_find_time_series(featureNames, 'MinorAxisLength');
    majorAxisLengthID = callback_chromodec_find_time_series(featureNames, 'MajorAxisLength');
    areaID = callback_chromodec_find_time_series(featureNames, 'Area');
    ...
    outputNames = callback_chromodec_generate_output_paths(parameters);
